...
const controllerDepositoNacional = {
	CargarVistaDeposito: async (req, res) => {
		const depositoNacional = await depositonacional.findAll({
			include: [
				{
					model: ciudad,
					include: [
		...
		res.render("Deposito", {
			...
			depositos: depositoNacional,
			tipoDeposito: "Depositos Nacionales",
			ubicaciones: ubicaciones,
			rutaAction: "DepositoNacional",
			cartel: "Deposito Nacional",
		});
	},
	RegistrarDepositoNacional: async (req, res) => {
		let mensajeToast = "";
		...
		if (Object.keys(errores).length === 0) {
			...
			//console.log(ubicacionARegistrar.id);

			const [, resultado] = await depositonacional.findOrCreate({
				where: {
			...
			const depositos = await depositonacional.findAll({
				include: [
					{
						model: ciudad,
						include: [
	...
	EditarDepositoNacional: async (req, res) => {
		try {
			const { nombreDeposito, correo, direccion, provinciaId, ciudadNombre } = req.body;

			const depositoEditado = await depositonacional.findByPk(req.params.id, {
				include: {
					model: ciudad,
					include: [
						{
...
module.exports = controllerDepositoNacional;
